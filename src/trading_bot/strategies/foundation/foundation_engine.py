...
class FoundationEngine:
    ...
    async def _create_signal_from_zone(
        ...
        try:
            ...
            logger.info(f"DEBUG: {symbol} Asset Class: {asset_class}, Pip Size: {pip_size}")
            ...
            if asset_class == "commodities":
                ...
                # Wide zones = imprecise, high risk. Max 350 pips ($3.50)
                max_acceptable_zone_width = 350.0  # pips
                ...
                # Only trade moderately strong zones. Min strength 0.5
            ...
            if asset_class == "crypto":
                ...
                # Max zone height: 100.0 pips ($10.00 on Gold) - Moderate cap
                max_zone_height_pips = 100.0
                max_zone_height = max_zone_height_pips * pip_size

                # Boost RR for commodities to allow runners
                rr_ratio = 5.0  # Target 5R ($50)
                ...
                # Ensure minimum SL distance (e.g. 20 pips / $2.00)
                min_sl_distance = 20.0 * pip_size
                if sl_buffer < min_sl_distance:
                    sl_buffer = min_sl_distance
            ...
            if is_demand:
                ...
                # HARD CAP SL DISTANCE FOR COMMODITIES
                if asset_class == "commodities":
                    max_sl_dist = 100.0 * pip_size  # 100 pips ($10.00) Cap
                    current_dist = entry_price - stop_loss
                    if current_dist > max_sl_dist:
                        stop_loss = entry_price - max_sl_dist
                        logger.debug(
                            f"{symbol}: SL capped at {max_sl_dist / pip_size:.1f} pips ($10.00) from entry"
                        )
                ...
                if asset_class == "commodities":
                    max_sl_dist = 100.0 * pip_size  # 100 pips ($10.00) Cap
                    current_dist = stop_loss - entry_price
                    if current_dist > max_sl_dist:
                        stop_loss = entry_price + max_sl_dist
                        logger.debug(
                            f"{symbol}: SL capped at {max_sl_dist / pip_size:.1f} pips ($10.00) from entry"
            ...
            # ═══════════════════════════════════════════════════════
            if asset_class == "commodities":
                min_score_threshold = 30.0  # Optimal balance: quality + frequency
                if final_score < min_score_threshold:
                    logger.warning(
                        f"{symbol}: REJECTED - Score too low ({final_score:.1f} < {min_score_threshold}). "
                        f"Need more confluence. Active layers: {list(layer_scores.keys())}"
                    )
                    return None
