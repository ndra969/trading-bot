...
class PartialCloseManager:
    ...
    def get_next_level(self, position: Position) -> PartialCloseLevel | None:
        ...
        if position.position_type.value == "BUY":
            risk_pips = (position.entry_price - position.stop_loss) / pip_size
            tp_distance_pips = (position.take_profit - position.entry_price) / pip_size
        else:  # SELL
            risk_pips = (position.stop_loss - position.entry_price) / pip_size
    ...
    def execute_partial_close(self, position: Position, close_price: float) -> dict[str, float]:
        ...
        if close_volume > remaining:
            logger.warning(f"Close volume {close_volume:.3f} exceeds remaining {remaining:.3f}, adjusting to remaining")
    ...
    def get_partial_close_levels(self, symbol: str, position: Position | None = None) -> list[PartialCloseLevel]:
        """
