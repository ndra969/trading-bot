...
async def test_valid_bullish_breakout(analyzer):
    ...
    opens, highs, lows, closes, volumes = create_breakout_data(
        (100.0, 105.0, 99.0, 104.0), 2000.0
    )
    
    signal = await analyzer.analyze_breakout(
        "EURUSD", opens, highs, lows, closes, volumes, 
        level_price=102.0, expected_direction='BULLISH'
    )
...
async def test_valid_bearish_breakout(analyzer):
    ...
    opens, highs, lows, closes, volumes = create_breakout_data(
        (100.0, 101.0, 95.0, 96.0), 1800.0
    )
    
    signal = await analyzer.analyze_breakout(
        "EURUSD", opens, highs, lows, closes, volumes, 
        level_price=98.0, expected_direction='BEARISH'
    )
...
async def test_no_breakout(analyzer):
    ...
    signal = await analyzer.analyze_breakout(
        "EURUSD", opens, highs, lows, closes, volumes, 
        level_price=105.0, expected_direction='BULLISH'
    )
