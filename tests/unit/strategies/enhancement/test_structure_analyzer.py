    ...
    for idx, val, ptype in points:
        if ptype == 'HIGH':
            ...
            for offset in range(1, 4):
                if idx - offset >= 0: highs[idx-offset] = val - 1
                if idx + offset < length: highs[idx+offset] = val - 1
        elif ptype == 'LOW':
            lows[idx] = val
            # Ensure adjacent points (window=3) are higher to create a swing low
            for offset in range(1, 4):
                if idx - offset >= 0: lows[idx-offset] = val + 1
                if idx + offset < length: lows[idx+offset] = val + 1
