...
from trading_bot.data.repositories import SessionRepository
...
class TestSessionRepository:
    ...
    async def test_update_session_error_handling(self, setup_test_database):
        ...
        # Patch get_session
        with patch("trading_bot.data.repositories.session_repository.get_session", mock_get_session):
...
class TestCloseAbandonedSessions:
    ...
    async def test_closes_only_active_sessions_for_account(self, setup_test_database):
        ...
        s1 = await repo.create({"session_id": "abandoned_1", "account_id": acc1.account_id, "status": "ACTIVE"})
        s2 = await repo.create({"session_id": "abandoned_2", "account_id": acc1.account_id, "status": "ACTIVE"})
        s3 = await repo.create({"session_id": "already_closed", "account_id": acc1.account_id, "status": "ACTIVE"})
        await repo.close_session("already_closed", 1100.0)
        # Account 2: 1 ACTIVE that must NOT be touched
        s4 = await repo.create({"session_id": "other_account", "account_id": acc2.account_id, "status": "ACTIVE"})
