...
import pytest
from trading_core.data.repositories import ConfigRepository
...
class TestConfigRepository:
    ...
    async def test_create_config_snapshot(self, setup_test_database):
        ...
        snapshot = await repo.create(
            config_data, description="Test config", environment="development"
        )
    ...
    async def test_delete_error_handling(self, setup_test_database, monkeypatch):
        ...
        # Patch get_session
        with patch(
            "trading_core.data.repositories.config_repository.get_session", mock_get_session
        ):
