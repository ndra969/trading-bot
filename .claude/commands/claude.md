...
# Display Project Rules (Claude Command)

This command is an alias for `/rules` and behaves identically. It provides an alternative way to access project rules, making it easier to remember since it directly references the CLAUDE.md file.
...
- `/claude` → Display complete CLAUDE.md
- `/claude full` → Display complete CLAUDE.md
- `/claude summary` → Display key sections summary
- `/claude rules-only` → Display only critical implementation rules

## Related

- `/rules` - Same functionality, different name
- CLI: `uv run trading-bot claude --format [format]`
